...
int getJulianDayNumber(int year, int month, int day)
{
    ...
    int a = (14 - month) / 12; // int a = month < 3 ? 1 : 0;
    int y = year + 4800 - a;
    int m = month + 12 * a - 3;
...
:julianDayNumber_(getJulianDayNumber(y, m, d)) {}  //no content

Date::Date(const struct tm& t)
:julianDayNumber_(getJulianDayNumber(t.tm_year + 1900, t.tm_mon + 1, t.tm_mday))  {}  //no content
